...
- **C2 (P1)** — ~~État vide de la boutique. Supprimer `disabled` sur `tab-left` : l'onglet ouvre toujours le drawer. Si `!shopUnlocked`, `ShopPanel` affiche à la place de la vitrine un état vide : « Pose d'abord un pari, le stagiaire n'ouvre pas la caisse aux indécis. » + bouton « Aller aux paris » (ouvre le drawer paris). Dès le premier pari posé, la vitrine apparaît sans re-clic si le panneau est ouvert.~~ **Révisé.** Le refus est remonté avant la porte au lieu d'être affiché derrière : **le bouton « Boutique » du pied du panneau de paris est `disabled` tant qu'aucun pari n'est posé** (désaturé, avec la même extinction que « Poser le pari »), son `title` disant pourquoi. Même intention — le stagiaire n'ouvre pas la caisse aux indécis — mais le joueur ne traverse plus un panneau vide pour l'apprendre.
  *Conséquence* : l'état vide de `ShopPanel` et son bouton « Aller aux paris » ne sont plus atteignables. Le code est conservé (il redeviendrait utile si une autre porte s'ouvrait sans pari) mais n'a plus de chemin depuis l'interface.
- **C2 bis** — **Une seule porte vers la boutique** : la poignée « Boutique · {n} objets » au-dessus de la piste a été retirée. On entre par le bouton du pied du panneau de paris, ou par le raccourci `B`.
- **C3 (P2)** — Ouverture exclusive, sans condition (révisé une première fois par 08/C2 « panneaux haut/bas », puis à nouveau — voir ci-dessous). **La boutique ouverte occupe l'écran seule : piste et panneau de paris sont démontés, pas seulement repliés.** Ouvrir l'un ferme l'autre, quel que soit le nombre de couloirs ou la taille de la fenêtre ; les clés `layout.bothPanelsMaxLanes` et `layout.bothPanelsMinHeight` de `config/race.json` n'ont plus d'objet et ont été retirées. *Raison* : acheter est un moment à part ; avec le plateau et le ticket derrière, la boutique était une troisième chose à lire au lieu de la seule. *Conséquence* : la poignée « Paris » disparaît tant que la boutique est là, donc **les trois sorties de la boutique (croix, « Retour aux paris », « Aller aux paris ») rouvrent toutes le panneau de paris** — sinon on se retrouve devant un écran sans panneau.
