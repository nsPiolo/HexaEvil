...
export function PlayerSlot({ ui, onStart, onRoll, onPickSoul, onPickDistance, onReset, onResolve }: PlayerProps) {
        ...
        <div className="dice-group">
          ...
          <div className="dice-row">
            {Array.from({ length: soulCount }, (_, i) => {
              ...
              return (
                <button key={i} type="button" className={cls.join(' ')} disabled={!pairing || order >= 0} onClick={() => onPickSoul(i)} style={id !== undefined && !rolling ? { ['--soul' as string]: soulColor(id) } : undefined}>
            ...
            {Array.from({ length: distCount }, (_, i) => {
              ...
              return (
                <span key={i} className="die-wrap">
                  <button type="button" className={cls.join(' ')} disabled={!pairing || selectedSoulDie === null || order >= 0} onClick={() => onPickDistance(i)} title={die?.name}>
